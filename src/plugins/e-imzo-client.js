...
var EIMZOClient = {
    ...
    CAPIWS.callFunction({
      ...
      if (data.success) {
        ...
        if (timestamper) {
          ...
          const login = 'm24site'
          const password = 'z023jmcftwsg'
          const formdata = new FormData()

          formdata.set('pkcs7', pkcs7)
          timestamper(data.signature_hex, async function(tst){
            const res = await fetch('https://api.multibank.uz/api/references/eimzo/timestamp/pkcs7', {
              body: formdata,
              method: 'POST',
              headers: {
  ...
  appendPkcs7Attached: async function (e, n, t, i, a) {
    const login = 'm24site'
    const password = 'z023jmcftwsg'

    const { signed_string, signature } = n
    const data64 = Base64.encode(signed_string)

    CAPIWS.callFunction({
      plugin: "pkcs7",
      name: "create_pkcs7",
      arguments: [data64, e, 'no'],
    }, function (e, n) {
      if (n.success) {
        var l = n.pkcs7_64
        if (t) {
          var s = n.signer_serial_number
          const formdata = new FormData()

          formdata.set('pkcs7', l)
          t(n.signature_hex, async function (e) {
            const res = await fetch('https://api.multibank.uz/api/references/eimzo/timestamp/pkcs7', {
              body: formdata,
              method: 'POST',
              headers: {
            ...
            if (!res.ok) {
              a(null, 'timestamp error')
            } else {
              const data = await res.json()
              const formdata = new FormData()

              formdata.set('data', `${signature}|${data.data}`)

              const resJoin = await fetch('https://api.multibank.uz/api/references/eimzo/pkcs7/join', {
                body: formdata,
                method: 'POST',
                headers: {
                  'Authorization': `Basic ${btoa(`${login}:${password}`)}`,
              ...
              i(joinData.data.pkcs7b64)
            }
          }, a)
        } else i(l)
      } else a(null, n.reason)
    }, function (e) {
      a(e, null)
    })
  },
